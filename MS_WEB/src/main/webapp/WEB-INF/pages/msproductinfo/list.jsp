	...
	<c:forEach items="${list}" var="item">
		...
		<td>${item.description }</td>
			...
			<td><a href="toupdatemsproduct?id=${item.id}">修改</a>||<a href="deletemsproductByid?id=${item.id}">删除</a>||<a href="querymsproductByid?id=${item.id}">查看</a>||<a href="toupdatemsproductstate?id=${item.id}">审核</a>
				||<a href="/msproductdetail/toinsertMsproductdetail?productid=${item.id}&&merchantid=${item.merchantid}">添加商品详情</a>||<a href="/msproductdetail/queryMsproductdetailByid?productid=${item.id}">查看商品详情</a>||<a href="/msproductdetail/toupdateMsproductdetail?productid=${item.id}">修改商品详情</a></td>
		</tr>
	</c:forEach>
</table>
